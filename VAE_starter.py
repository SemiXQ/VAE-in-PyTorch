...
def Plot_Kernel(_model):
    ...
    kernel = _model.encoder.fc.weight
    kernel = kernel.reshape((kernel.shape[0], 28, 28))
    img = kernel.tolist()
    num = kernel.shape[0] / 5
    for i in range(0, int(num-1)):
        plt.subplot(2, 3, i+1)
        plt.title("Kernel[%d]" % (i*5))
        img = kernel[i*5].tolist()
        plt.imshow(img)
    plt.show()
    return
...
def scatter_plot_TSNE(latent_representations, labels):
    after_tsne = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(latent_representations)
    scatter_plot(latent_representations=after_tsne, labels=labels)
...
class VAE_Trainer(object):
    ...
    def loss_function(self, recon_x, x, mu, logvar, bseweight, kldweight):
        # Note that this function should be modified for the VAE part.
        # KLD term should be added to the final Loss.

        # nn.KLDivLoss return strange result (maybe it is because the original distribution is not N(0,1), not sure)
        # I use the simplified formular calculate KL(N(mu,std^2)||N(0,1))
        # the formular: KL(N(mu,std^2)||N(0,1))= 0.5*(-log(std^2)+mu^2+std^2-1)
        kld = torch.sum(-logvar + torch.pow(mu, 2) + torch.exp(logvar) - 1.0) * 0.5
        BCE = F.mse_loss(recon_x, x)
        Loss = bseweight * BCE + kldweight * kld
        return Loss
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, _) in tqdm(enumerate(self.train_loader), total=len(self.train_loader)):
            ...
            recon_batch, mu, logvar, bseweight, kldweight = self.model(data)
            loss = self.loss_function(recon_batch, data,  mu, logvar, bseweight, kldweight)
            loss.backward()
            train_loss += loss.item()
            self.optimizer.step()

        train_loss /= len(self.train_loader.dataset)/32  # 32 # 32 is the batch size
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, (data, _) in tqdm(enumerate(self.val_loader), total=len(self.val_loader)):
                data = data.to(self.device)
                recon_batch,  mu, logvar, bseweight, kldweight = self.model(data)
                val_loss += self.loss_function(recon_batch, data,  mu, logvar, bseweight, kldweight).item()
